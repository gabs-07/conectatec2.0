...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['comentario'])) {
        ...
        if (!empty($comentario)) {
            try {
                ...
                $query = $dbConnection->prepare("INSERT INTO comentariosBD (usuario, comentario, fecha) VALUES (:usuario, :comentario, NOW())");
                $query->execute([
                    'usuario' => $usuario,
    ...
    try {
        $query = $dbConnection->prepare("SELECT * FROM comentariosBD ORDER BY fecha DESC");
        $query->execute();
...
<body>
    ...
    <div>
        ...
        <form action="eventoOneBd.php" method="post">
