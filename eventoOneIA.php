...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['comentario'])) {
        ...
        if (!empty($comentario)) {
            try {
                ...
                $query = $dbConnection->prepare("INSERT INTO comentarios (usuario, comentario, fecha) VALUES (:usuario, :comentario, NOW())");
                $query->execute([
                    'usuario' => $usuario,
    ...
    try {
        $query = $dbConnection->prepare("SELECT * FROM comentarios ORDER BY fecha DESC");
        $query->execute();
...
<head>
    ...
    <title>Explorando la IA - IA</title>
</head>
<body>
    ...
    <div>
        ...
        <form action="eventoOneIA.php" method="post">
